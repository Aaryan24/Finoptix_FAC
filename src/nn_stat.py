...
print('STATIC-ONLY ablation: no sequence branch');print("samples",S.shape,"dates",len(set(DT)),flush=True)

class Net(nn.Module):
    def __init__(self,nf=3,ns=len(FN),hid=32,emb=16,p=0.3):
        super().__init__()
        self.lstm=nn.LSTM(nf,hid,batch_first=True)  # unused
        self.proj=nn.Linear(hid,emb)
        self.head=nn.Sequential(nn.Linear(ns,32),nn.ReLU(),nn.Dropout(p),nn.Linear(32,1))
    def forward(self,s,st):
        _,(h,_)=self.lstm(s)
        return self.head(st).squeeze(-1)
...
R=pd.concat(rows); R.to_pickle('nn_statonly.pkl')
...
json.dump({'ic':float(m),'t':float(m/ic.std()*np.sqrt(n)),'n':int(n)},open('nn_statonly.json','w'))

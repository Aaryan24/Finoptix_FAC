...
for f in [pd.Timestamp(x) for x in pd.date_range('2021-01-01','2026-01-01',freq='12MS')]:
    ...
    for d in te:
        ...
        rows.append(pd.DataFrame({'date':d,'tic':tk,'pred':np.mean([p[d] for p in P],0),
                                  'y':yy[:,HS.index(HMAIN)]}))
    pd.concat(rows).to_pickle('xattn_partial.pkl')          # checkpoint after every fold
...
R=pd.concat(rows); R.to_pickle('xattn_pred.pkl')
...
json.dump({'ic':float(ic.mean()),'t':float(ic.mean()/ic.std()*np.sqrt(len(ic))),'n':int(len(ic))},open('xattn_ic.json','w'))

...
H=21; EMBARGO=45; MIN_ADV=5e7          # Rs 5 crore median daily traded value
...
F['turn']    =_dv.rolling(21,min_periods=10).mean()/_dv.rolling(252,min_periods=120).mean()
...
for s in starts:
    ...
    o=te[['date','tic','y']].copy(); o['pred']=comp.values; out['composite'].append(o)
    rg=Ridge(alpha=50.0).fit(tr[FN],tr['y'])
    o=te[['date','tic','y']].copy(); o['pred']=rg.predict(te[FN]); out['ridge'].append(o)
    ...
    gb.fit(tr[FN],tr['y'])
    o=te[['date','tic','y']].copy(); o['pred']=gb.predict(te[FN]); out['xgb'].append(o)
...
for m,v in out.items():
    if not v: continue
    df=pd.concat(v); df.to_pickle(f'v3fix2_{m}.pkl')
...
json.dump({'single':{k:{kk:vv for kk,vv in v.items() if kk!='ic_series'} for k,v in single.items()},
           'models':res}, open('v3fix2_results.json','w'), indent=2, default=str)
